    ...
    <div className='bg-white mb-5 border-2 border-black border-solid w-full p-3 rounded-md relative'>
        <div className='flex space-x-8'>
            ...
            <div className='ml-3 w-[50%] py-4'>
                <div className={`h-[55%] font-bold text-3xl ${props.index % 2 !== 0 ? "text-right" : ""}`}>{props.data.name}</div>
                <div className={`h-[20%] text-xl ${props.index % 2 !== 0 ? "text-right" : ""}`}>Rp {props.data?.totalPrice?.toLocaleString("id-ID")}</div>
                <div className={`h-[20%] flex ${props.index % 2 !== 0 ? "justify-end" : ""} space-x-3`}>
